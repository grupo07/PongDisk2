...
public class PongView extends View implements View.OnTouchListener, View.OnKeyListener {
   ...
   //estado inicial de los botones y disco
    ...
    protected void loadPreferences(SharedPreferences prefs) {
        ...
        mCpuHandicap = Math.max(0, Math.min(PLAYER_PADDLE_SPEED+9, prefs.getInt(PongDisk.PREF_HANDICAP, 4)));
    ...
    public void onDraw(Canvas canvas) {
       ...
       //si no estan jugando esto les hace dar a entender que no juegan pero se pueden unir.
        ...
        //codigo para linea medio campo
        String linea = context.getString(R.string.pause2);
        int pausew = (int) mPaint.measureText(linea);
        canvas.drawText(linea,getWidth()/2-pausew/2,(getHeight()+20)/2,mPaint);
        //dibujar circulo en el medio
        canvas.drawOval(15,15,15,15,mPaint);
        /////////
        ...
        if(!gameRunning()) {
            ...
            if(!mBlue.living()) {
                ...
                int height = (int) (mPaint.ascent() + mPaint.descent());
                canvas.drawText(s, getWidth() / 2 - width / 2, getHeight() / 2 - height / 2, mPaint);
                String espera=context.getString(R.string.wait);
                canvas.drawText(espera, getWidth()/2-pausew/2,(getHeight()+80)/2,mPaint);
                esperarNewgame(5000);
                //Toast toast= Toast.makeText(context,"Esperar 5 segundos para nuevo juego", Toast.LENGTH_SHORT);
                //toast.show();
    ...
    //esperar a que funciones
    public void esperarNewgame(int milisegundos) {
    ...
    private void resetPaddles() {
        ...
        mRed.setLives(STARTING_LIVES+2);
        mBlue.setLives(STARTING_LIVES+2);
    }
    ...
    class Paddle {
        ...
        //grosor del palo
        private static final int PADDLE_THICKNESS = 5;
